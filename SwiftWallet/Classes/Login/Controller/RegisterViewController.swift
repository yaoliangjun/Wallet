...
class RegisterViewController: BaseViewController {
    ...
    @objc func registerBtnClick() {
        ...
        LoginServices.register(params: params, showHUD: true, success: { (response) in
            MBProgressHUD.show(withStatus: NSLocalizedString("注册成功", comment: ""), completionHandle: {
    ...
    @objc func verifyCodeBtnClick() {
        ...
        LoginServices.verifyCodeForRegister(params: params, showHUD: true, success: { (response) in
            // 保存点击时候的时间
    ...
    override func setupSubViews() {
        ...
        // 注册
        let registerBtn = CommonButton(title: NSLocalizedString("注册", comment: ""), target: self, selector: #selector(registerBtnClick))
        ...
        contentView.snp.makeConstraints { (make) in
            make.bottom.equalTo(registerBtn.snp.bottom).offset(10)
        }
    }
}
